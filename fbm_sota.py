...
average_likelihood = tf.reduce_mean(likelihood, axis=0)
...
def compute_log_prob(sample_size=100):
    data = generate_data(sample_size, sample_length=sample_length, H=H)
    with tf.Session() as sess:
        code_sample = sess.run(prior.sample())
        code_sample = np.expand_dims(code_sample, axis=0)
        saver.restore(sess, "./version_" + str(version))
        log_prob = sess.run(average_likelihood, feed_dict={code: code_sample, X: data})
    return log_prob

print() 
print("Test log_probability equals: ", compute_log_prob())